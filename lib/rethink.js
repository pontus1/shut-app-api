 ...
 */
module.exports.getConversationsByParticipant = function (participantId) {
  return rdb.table('conversations').filter(function(conversation) {
      return conversation("participants").contains(participantId)
    }).run()
    .then(function (cursor) {
      return cursor.toArray()
    })
